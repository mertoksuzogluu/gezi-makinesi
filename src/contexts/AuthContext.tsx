...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  const getStoredUsers = (): StoredUser[] => {
    ...
    // Merge existing users with demo users (ensure admin exists)
    const existingUsers = JSON.parse(stored) as StoredUser[];
    const adminExists = existingUsers.some(u => u.email === 'admin@vizelyio.com');
    if (!adminExists) {
      const mergedUsers = [...existingUsers, demoUsers[1]]; // Add admin user
